...
import { Search, Plus, Edit3, Trash2, Building2, X, Save, RefreshCw } from 'lucide-react';
...
export const SupplierManager = () => {
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            if (editingSupplier) await api.suppliers.update(editingSupplier.id, formData, currentUser);
    ...
    const handleToggleStatus = async (s: Supplier) => {
        ...
        try { 
            await api.suppliers.update(s.id, { ...s, status: newStatus }, currentUser); 
            fetchSuppliers(); 
        } catch (err: any) { 
            alert(err.message); 
        }
    ...
    return (
        <div className="space-y-8 animate-in fade-in">
            <div className="flex justify-between items-end">
                <div>
                    <h2 className="text-4xl font-black text-gray-800 uppercase tracking-tight">Nhà cung cấp</h2>
                    <p className="text-xs text-gray-400 font-bold uppercase tracking-widest mt-1">Đối tác nguồn hàng SportHub</p>
                </div>
                <button 
            ...
            <div className="bg-white rounded-[40px] shadow-sm border border-gray-100 overflow-hidden">
                <table className="w-full text-left">
                    ...
                    <tbody className="divide-y divide-gray-50">
                        {filteredSuppliers.map(s => (
                            <tr key={s.id} className={`hover:bg-gray-50 transition group ${s.status === 'inactive' ? 'opacity-50' : ''}`}>
                                <td className="px-10 py-6">
                                    <p className="font-black text-gray-800 uppercase text-base">{s.name}</p>
                                    <p className="text-[10px] font-bold text-gray-400 mt-1 uppercase tracking-widest">MST: {s.taxCode || 'N/A'}</p>
                                </td>
                                <td className="px-6 py-6 font-black text-gray-600 text-sm uppercase">{s.contactPerson}</td>
                                <td className="px-6 py-6">
                                    <p className="text-xs font-black text-gray-700">{s.phone}</p>
                                    <p className="text-[10px] font-bold text-gray-400 mt-0.5">{s.email || ''}</p>
                                </td>
                ...
                {filteredSuppliers.length === 0 && !loading && (
                    <div className="py-20 text-center text-gray-400 uppercase font-black text-xs tracking-widest">Không tìm thấy nhà cung cấp nào</div>
                )}
            </div>

            {isModalOpen && (
                <div className="fixed inset-0 bg-black/60 z-[300] flex items-center justify-center p-4 backdrop-blur-md">
                    <form onSubmit={handleSave} className="bg-white rounded-[40px] w-full max-w-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95">
                        <div className="bg-gray-50 p-8 border-b border-gray-200 flex justify-between items-center">
                            <div className="flex items-center gap-4">
                                ...
                                <h2 className="text-xl font-black uppercase tracking-tight">
                                    {editingSupplier ? 'Cập nhật đối tác' : 'Thêm nhà cung cấp mới'}
                                </h2>
                            </div>
                            <button type="button" onClick={() => setIsModalOpen(false)} className="p-2 text-gray-400 hover:text-red-500 transition">
                                <X size={24}/>
                            </button>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-10 space-y-8 custom-scrollbar">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                ...
                                <InputField 
                                    ...
                                    onChange={(v:string)=>setFormData({...formData, taxCode: v})} 
                                />
                            </div>
                        ...
                        <div className="p-8 bg-gray-50 border-t flex justify-end gap-4 shadow-2xl">
                            ...
                            >
                                {loading ? <RefreshCw className="animate-spin" size={16} /> : <><Save size={18}/> LƯU ĐỐI TÁC</>}
                            </button>
                        </div>
                    </form>
                </div>
