...
export const InventoryReportManager = ({
  onGoToStockEntry,
}: {
  onGoToStockEntry: () => void;
}) => {
  ...
  const [sortConfig, setSortConfig] = useState<{
    key: string;
    direction: "asc" | "desc";
  } | null>({ key: "stockQuantity", direction: "asc" });
  ...
  useEffect(() => {
    console.log(
      "📦 [InventorySystem] Loading inventory data with filters:",
      filters
    );
    setLoading(true);
    api.reports
      ...
      .then((res) => {
        console.log("✅ [InventorySystem] Data loaded:", res);
        setReportData(res);
        setLoading(false);
      })
      .catch((error) => {
        console.error("❌ [InventorySystem] Error loading inventory:", error);
        setLoading(false);
      });
  }, [filters]);
  ...
  return (
    <div className="space-y-10 animate-in fade-in text-slate-900 p-6 md:p-8 w-full">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-xs text-gray-400 font-bold uppercase mt-1 tracking-widest">
            Phân tích giá trị & Vốn lưu động
          </p>
        </div>
        <button
          onClick={() =>
            alert("Chức năng xuất báo cáo Excel đang được khởi tạo")
          }
          className="px-8 py-4 bg-slate-900 text-white rounded-3xl font-black text-xs uppercase shadow-xl flex items-center gap-3 hover:bg-black transition"
        >
      ...
      {reportData && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <StatCard
            ...
            value={`${reportData.metrics.totalValue.toLocaleString()}đ`}
            color="bg-blue-600"
          />
          <StatCard
            icon={<AlertCircle size={24} />}
            title="Sản phẩm sắp hết"
            value={`${reportData.metrics.lowStockCount} SKU`}
            color="bg-red-500"
          />
          <StatCard
            icon={<Package size={24} />}
            title="Tổng số lượng thực tế"
            value={`${reportData.metrics.totalItems.toLocaleString()} cái`}
            color="bg-slate-800"
          />
        </div>
      )}
        ...
        <table className="w-full text-left">
          <thead className="bg-gray-50 text-[10px] font-black text-slate-500 uppercase tracking-widest border-b border-gray-100">
            <tr>
              ...
              <th
                className="px-6 py-6 text-center cursor-pointer"
                onClick={() =>
                  setSortConfig({
                    key: "stockQuantity",
              ...
              >
                Tồn thực tế{" "}
                <ArrowDownUp size={12} className="inline ml-1 text-slate-300" />
              </th>
            ...
            {sortedList.length > 0 ? (
              sortedList.map((item: any, idx: number) => (
                ...
                >
                  ...
                  <td className="px-6 py-6 text-center text-xl font-black text-slate-900">
                    {item.stockQuantity}
                  </td>
                  <td className="px-6 py-6 text-right font-bold text-slate-600 text-xs">
                    {item.costPrice.toLocaleString()}đ
                  </td>
                  <td className="px-6 py-6 text-right font-black text-slate-800">
                    {item.inventoryValue.toLocaleString()}đ
                  </td>
                  <td className="px-8 py-6 text-center">
                    {item.isLowStock && (
                      <button
                        onClick={onGoToStockEntry}
                        className="px-4 py-2 bg-red-600 text-white rounded-xl text-[9px] font-black uppercase shadow-lg shadow-red-500/20 hover:scale-105 transition"
                      >
                        Lập phiếu nhập
                      </button>
                    )}
...
export const StocktakeManager = () => {
  ...
  const [currentStocktake, setCurrentStocktake] = useState<Stocktake | null>(
    null
  );
  ...
  const handleConfirmBalance = async () => {
    ...
    try {
      ...
      await api.inventory.saveStocktake(
      ...
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-xs text-gray-400 font-bold uppercase tracking-widest mt-1">
            Danh sách các đợt đã cân bằng kho
          </p>
        </div>
        <button
          onClick={handleStart}
          disabled={loading}
          className="px-10 py-4 bg-secondary text-white rounded-2xl font-black text-xs uppercase shadow-xl flex items-center gap-3 hover:bg-blue-600 transition disabled:opacity-50"
        >
          {loading ? (
              ...
              stocktakes.map((stk) => (
                <tr key={stk.id} className="hover:bg-gray-50 transition group">
                  ...
                  <td className="px-6 py-6 font-bold text-slate-600 text-xs">
                    {new Date(stk.date).toLocaleDateString("vi-VN")}
                  </td>
                  ...
                  <td className="px-10 py-6 text-right">
                    <button
                      onClick={() => handleOpenDetail(stk)}
                      className="p-3 text-slate-300 hover:text-secondary transition hover:bg-white rounded-xl shadow-sm border border-transparent hover:border-gray-100 transform group-hover:scale-110"
                    >
                      <Eye size={18} />
                    </button>
                  </td>
          ...
          <div className="bg-white rounded-[40px] w-full max-w-6xl shadow-2xl overflow-hidden p-0 flex flex-col max-h-[95vh] animate-in zoom-in-95">
              ...
              <div>
                <h2 className="text-2xl font-black uppercase text-slate-800 tracking-tight">
                  {currentStocktake.id === "new"
                    ...
                    : `Phiếu kiểm kê: ${currentStocktake.code}`}
                </h2>
                <p className="text-[10px] font-black text-slate-400 uppercase mt-1 tracking-widest">
                  Nhân viên thực hiện:{" "}
                  <span className="text-slate-900">
                    {currentStocktake.auditorName}
                  </span>
                </p>
              </div>
              <button
                onClick={() => setIsModalOpen(false)}
                className="p-3 text-slate-400 hover:text-red-500 hover:bg-white rounded-2xl transition shadow-sm border border-transparent hover:border-gray-100"
              >
                <X size={24} />
              </button>
            </div>
                ...
                <tbody className="divide-y divide-gray-50 text-slate-900">
                  {currentStocktake.items.map((item: any) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-6 text-center font-black text-lg text-slate-400">
                        {item.systemStock}
                      </td>
                      <td className="px-6 py-6 text-center">
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            const updated = currentStocktake.items.map(
                              (i: any) =>
                                i.id === item.id
                                  ? {
                                      ...i,
                                      actualStock: val,
                                      discrepancy: val - i.systemStock,
                                    }
                                  : i
                            );
                            setCurrentStocktake({
                      ...
                      <td
                        className={`px-6 py-6 text-center font-black text-lg ${
                          item.discrepancy === 0
                            ? "text-slate-200"
                            : item.discrepancy > 0
                            ? "text-green-500"
            ...
            <div className="p-8 bg-gray-50 border-t border-gray-200 flex flex-col md:flex-row justify-between items-center gap-6">
              ...
              <div className="flex gap-4">
                <button
                  onClick={() => setIsModalOpen(false)}
                  className="px-10 py-4 font-black text-slate-400 uppercase text-[10px] tracking-widest hover:text-slate-800 transition"
                >
                  Đóng cửa sổ
                </button>
                {currentStocktake.id === "new" && (
          ...
          <div className="bg-white rounded-[40px] w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95">
            <div className="p-10 text-center space-y-6">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-slate-500 font-medium leading-relaxed">
                  Dữ liệu thực tế sẽ được cập nhật lên Website và phiếu này sẽ
                  được lưu vào lịch sử kiểm kê. Bạn chắc chắn muốn tiếp tục?
                </p>
              </div>
            ...
            <div className="p-4 bg-gray-50 border-t border-gray-100 text-center">
              <p className="text-[8px] font-black text-slate-300 uppercase tracking-widest italic">
                SportHub Warehouse System v3.5
              </p>
            </div>
          </div>
        </div>
