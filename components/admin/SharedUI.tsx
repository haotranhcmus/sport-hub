...
export const InputField = ({ label, value, onChange, type = 'text', placeholder, disabled, required, suffix }: any) => (
    <div className="flex-1 min-w-[200px]">
        <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">{label} {required && '*'}</label>
        <div className="relative">
            <input 
                ...
                className={`w-full border rounded-2xl px-5 py-4 text-sm font-black outline-none focus:ring-2 focus:ring-secondary/10 transition ${
                    disabled ? 'bg-gray-100 border-gray-100 text-gray-400 cursor-not-allowed' : 'bg-gray-50 border-gray-200 text-slate-900 focus:bg-white focus:border-secondary/40'
                }`}
                value={value}
                onChange={e => onChange?.(type === 'number' ? parseFloat(e.target.value) || 0 : e.target.value)}
            />
            {suffix && <span className="absolute right-5 top-1/2 -translate-y-1/2 text-[10px] font-black text-gray-400 uppercase">{suffix}</span>}
        </div>
    </div>
);

export const TabButton = ({ id, active, onClick, label, icon }: { id: string, active: string, onClick: any, label: string, icon: any }) => (
    <button onClick={() => onClick(id)} className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-[11px] font-black uppercase tracking-widest transition ${active === id ? 'bg-white text-secondary shadow-md' : 'text-gray-400 hover:bg-white/50 hover:text-gray-600'}`}>
        {icon} {label}
    </button>
);
