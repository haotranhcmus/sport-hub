...
import { X, Trash2, ShoppingBag, Plus, Minus, ArrowRight, AlertTriangle } from 'lucide-react';
import { useCart } from '../../../context/CartContext';

export const CartDrawer = () => {
  const { isOpen, closeCart, items, removeFromCart, updateQuantity, totalPrice, isValid } = useCart();
      ...
      <div className="relative w-full max-w-md bg-white shadow-2xl h-full flex flex-col animate-in slide-in-from-right duration-500">
        ...
        <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-white z-10">
          <h2 className="text-xl font-black flex items-center gap-3 text-gray-800 uppercase tracking-tight">
            ...
            GIỎ HÀNG <span className="text-gray-400 text-xs font-bold tracking-widest">({items.length})</span>
          </h2>
          <button 
            onClick={closeCart} 
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
          {items.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-gray-500 space-y-6 text-center">
              ...
              <div>
                <p className="text-lg font-black text-gray-800 uppercase tracking-tighter">Trống không</p>
                <p className="text-sm text-gray-400 font-medium mt-1">Bắt đầu chọn đồ ngay nào!</p>
              </div>
              <button 
            ...
            items.map((item: any) => {
              ...
              return (
                <div key={item.variantId} className={`flex gap-4 p-4 rounded-2xl border transition ${!item.isAvailable ? 'bg-red-50/50 border-red-100' : 'border-gray-50 hover:shadow-md'}`}>
                  ...
                  <div className="flex-1 flex flex-col justify-between">
                    <div>
                      <h3 className={`font-black text-sm line-clamp-1 ${!item.isAvailable ? 'text-gray-400' : 'text-gray-800'}`}>
                        {item.product.name}
                      </h3>
                      ...
                      {(item.error || item.warning) && (
                          <p className="text-[8px] font-bold text-red-500 mt-1 uppercase tracking-tighter flex items-center gap-1">
                              <AlertTriangle size={10}/> {item.error || item.warning}
                          </p>
                      )}
                    </div>
                    <div className="flex justify-between items-end mt-2">
                      ...
                      <div className="text-right">
                        <p className={`text-sm font-black ${!item.isAvailable ? 'text-gray-400' : 'text-secondary'}`}>
        ...
        {items.length > 0 && (
          <div className="p-8 border-t border-gray-100 bg-gray-50 space-y-6 shadow-[0_-10px_30px_-15px_rgba(0,0,0,0.1)]">
            <div className="flex justify-between items-end">
              <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Tổng tạm tính:</span>
              <span className="text-2xl font-black text-red-600 tracking-tighter">{totalPrice.toLocaleString()}đ</span>
            </div>
